...
This analysis asks: **how many distinct utility function architectures does the population actually need?** It proceeds as follows: for M = 1, 2, 3, …, find the library of M utility function architectures such that, when every participant is free to use the architecture that fits them best, the total BIC cost across the population is minimized. The result is a **compression curve**: as M grows, the curve tracks what fraction of the fully individualized advantage (giving every participant their own unique best architecture) is recovered. The knee of this curve — the point where adding another architecture stops delivering meaningful improvement — is the answer to how many functional types are needed to describe the population.
...
2. **Are the selected architectures meaningfully behaviorally distinct?** — answered by AMPD (Average Model Policy Distance) computed on the selected library after the curve identifies the winning M.
...
- **M-architecture set** (also: *library of M architectures*): the set of M architectures available to the population. Each participant selects the architecture that fits them best (by BIC score).
...
- **Population BIC score at M**: the sum, across all participants, of each participant's minimum BIC given the M-architecture library. This is the quantity being minimized.
- **A(M)**: the proportion of the fully individualized advantage captured by a M-architecture library. A(1) = 0 by definition (no individualization possible with a single shared architecture). A(∞) = 1 (everyone has their own architecture). The compression curve plots A(M) vs M.
...
- **Kneedle elbow**: the M at which the compression curve has the greatest vertical distance above the straight line connecting its two endpoints (M=1 and M=M_max). This is the formal, threshold-free definition of the "knee" of the curve.
...
Penalizes with each participant's own game count. This can over-penalize complex models relative to the IC analysis: `Σᵢ k × log(n_i)` is larger than `k × log(Σᵢ n_i)` because log is concave. May produce a different M=1 winner than the IC champion.
...
This guarantees that M=1 selects the same winner as the IC analysis. If the M=1 winner differs despite using this basis, the discrepancy must be attributed to candidate filtering or role inclusion differences — not the scoring formula itself.

**Self-check at startup:** After identifying the M=1 winner, compare it to the IC CSV winner. If they match: print a confirmation. If they do not match: print a detailed diagnostic (which model was selected, which was expected, what their scores are) and continue — do not halt, as a mismatch can itself be scientifically informative.
...
- **M=1 score** (`score_K1`): find the single model that minimizes `L.sum(axis=0)`. This is the best single-architecture solution. Self-check against IC CSV winner (see above).
- **Fully individualized score** (`score_fully_individualized`): `L.min(axis=1).sum()` — each participant uses their personal best model. Also compute `n_unique_individual_best_models` = number of distinct models selected across participants, and `M_useful_max` = that count. Adding more models beyond `M_useful_max` cannot improve `score_fully_individualized` under optimal search.

### Step 3 — Find optimal M-architecture set for each M

**Exhaustive search (M ≤ exhaustive_M_max, default 4):**

Enumerate all C(N_candidates, M) possible sets. For each set S: `score(S) = L[:, S].min(axis=1).sum()`. Operations are batched (100,000 combinations per batch) to control memory (~200 MB per batch at M=4). Parallelized across `n_workers` independent slices using `mp.Pool`; each worker searches its slice and returns its local best.

Feasibility with N=120: M=2 (7k combos, milliseconds), M=3 (280k, ~1s), M=4 (8.1M, ~30s parallelized). With N=240: M=4 is 135M — hours; consider lowering `exhaustive_M_max` to 3.

**Greedy + local swap (M > exhaustive_M_max):**

Starting from the winning set at M-1:
...
### Step 4 — Compute A(M) and all stopping criteria

```
A(M) = (score_K1 - score_M) / (score_K1 - score_fully_individualized)
ΔA(M) = A(M) − A(M−1)         [first finite difference; nan at M=1]
Δ²A(M) = ΔA(M) − ΔA(M−1)     [second finite difference; nan at M=1,2]
```

Compute all four stopping criteria (see Stopping Criteria section). Flag the selected M for each criterion in output columns. The `stopping_criteria` parameter only controls which criterion is highlighted in terminal output and plots — all criteria are always computed.

### Step 5 — AMPD diagnostics on the selected library

After computing the full curve, load the AMPD matrix (if available) and compute library-level AMPD statistics for each M:
...
For each architecture in each M-set, compute per-architecture diagnostics that flag potential redundancy. **These are diagnostics only — the function does not automatically remove models.**

For each model m in the selected M-architecture set:
...
After each M completes, immediately append the result row to a partial CSV (`population_architecture_curve_partial.csv`). On startup with `create_new_file=False`, if the partial file exists and has valid rows, skip the already-completed M values and resume. On successful completion, write the three final CSVs and delete the partial file.

Print a one-line summary per M to the terminal, including ETA using `_fmt_duration` (analysis.py ~line 6166).
...
The Kneedle elbow is the closest implementation of the visual intuition: find the point where the compression curve bends away from the straight line between its two endpoints (M=1, A=0) and (M_max, A(M_max)).

**Procedure:**
1. Normalize M to [0, 1]: `M_norm = (M - 1) / (M_max - 1)`
2. Normalize A(M) to [0, 1]: `A_norm = A(M) / max(A(M))`
3. For each M, compute: `kneedle_distance(M) = A_norm(M) - M_norm(M)`
4. Select: `M* = argmax_M kneedle_distance(M)`

**Why defensible:** Directly formalizes the visual intuition of "the bend in the curve." No threshold required — the optimal M is chosen automatically by maximizing departure from linearity. Well-established in clustering literature (the "elbow method" for M-means).

**Limitation:** Requires choosing M_max to define the normalization range; results can shift if M_max changes.

### Marginal Gain (ΔA < threshold)

```
first_M_with_low_marginal_gain = min M > 1 where ΔA(M) < marginal_gain_threshold
selected_M_by_marginal_gain = first_M_with_low_marginal_gain - 1
```

Note the off-by-one: if M=5 is the first M where the marginal gain falls below threshold, the fifth architecture *failed* to earn its place, so the answer is M=4.
...
### Cumulative Gain (A(M) > threshold)

```
selected_M_by_cumulative_gain = min M where A(M) > cumulative_gain_threshold
```
...
```
selected_M_by_max_curvature = argmax_M |Δ²A(M)|    for M ≥ 2
```

The second finite difference approximates the curvature of the compression curve. Since A(M) is concave by construction, Δ²A ≤ 0 everywhere; the knee is where curvature is most negative (largest absolute value = sharpest deceleration of improvement).

**Why defensible:** The discrete analog of finding the inflection point of the cumulative gain curve. No threshold required.

**Limitation:** Sensitive to noise; less reliable when improvement is spread gradually across many M values.

### Meta-BIC (exploratory only)

```
exploratory_meta_bic(M) = 2 × bic_score_M + M × mean_k_effective_in_set × log(N_participants)
selected_M_by_meta_bic = argmin_M exploratory_meta_bic(M)
```

Treats M itself as a model complexity parameter and penalizes it with BIC logic. Named `exploratory_meta_bic` to signal that this formula is heuristic (the 2× factor may double-count complexity if `bic_score_M` already incorporates BIC penalties). Useful cross-check; not the primary criterion.
...
`M_max=None` means run until the selected stopping criterion is met, or until all candidate models are exhausted. `n_workers` is clamped to `[1, cpu_count - 1]`; `None` = `cpu_count - 1`.

Returns one DataFrame (one row per M). The old function returned a tuple; this no longer has a fold structure.
...
One row per M. Columns include: M, architecture_set_idxs (JSON), search_method, score_basis, raw_nll_score_M, sum_individual_BIC_score_M, ic_equivalent_score_M, score_K1, score_fully_individualized, A_M, delta_A_M, delta2_A_M, M_useful_max, n_unique_individual_best_models, kneedle_distance, selected_by_kneedle_elbow, first_M_with_low_marginal_gain, selected_M_by_marginal_gain, selected_by_marginal_gain, selected_by_cumulative_gain, selected_by_max_curvature, exploratory_meta_bic, selected_by_meta_bic, library_ampd_min, library_ampd_mean, library_ampd_median, library_ampd_max, nearest_selected_pair_by_ampd.

### `processed/population_architecture_assignments.csv`
One row per M × participant. Columns: M, player_uuid, assigned_utility_idx, assigned_model_rank_for_player, assigned_model_delta_score_for_player, assigned_model_AMPD_to_population_winner, assigned_model_AMPD_to_nearest_selected_model.

### `processed/population_architecture_library_diagnostics.csv`
One row per M × selected architecture. Columns: M, utility_idx, equation, k_params, utility settings columns, assigned_n, assigned_percent, pruning_cost, pruning_cost_normalized, nearest_selected_model_idx, nearest_selected_model_ampd, nearest_selected_model_ampd_percentile, redundancy_warning_level, redundancy_score_optional.

### `processed/population_architecture_curve_partial.csv`
Partial results; appended after each M completes. Deleted on successful completion. Enables resume after interruption.
...
Replaces the old function of the same name. Reads `processed/population_architecture_curve.csv` (one row per M, no fold structure) and produces an interactive Plotly figure exported to `visuals/population_architecture_curve.html`.

**Key differences from the old function:**
- No error bars (single BIC score per M, no fold SE).
- Hover text shows: M, A(M), ΔA(M), Δ²A(M), kneedle_distance, architecture_set_idxs (JSON list of model indices).
- Vertical knee markers for **all five** stopping criteria, each in a distinct color/dash style:
  ...
  Only criteria whose selected M is non-null are drawn; if two criteria agree on the same M, a single combined annotation is used.
...
- Horizontal reference lines at A=0 (M=1 baseline) and A=1 (fully individualized).
- Reads columns from the new CSV: `M`, `A_M`, `delta_A_M`, `delta2_A_M`, `kneedle_distance`, `architecture_set_idxs`, `selected_by_kneedle_elbow`, `selected_by_marginal_gain`, `selected_by_cumulative_gain`, `selected_by_max_curvature`, `selected_by_meta_bic`.
...
- [ ] M=1 primary score reproduces population IC winner; if not, diagnostic output explains why
- [ ] Candidate set always includes the population IC winner and every participant's personal best model
- [ ] A(M) is monotonic non-decreasing; score_M is monotonic non-increasing
- [ ] The curve reaches `score_fully_individualized` by M_useful_max under exhaustive/global search
- [ ] Marginal-gain off-by-one correction implemented: `selected_M_by_marginal_gain = first_M_with_low_marginal_gain - 1`
...
- [ ] AMPD diagnostics computed after library selection (not used as primary M-selection criterion)
...
## Appendix: The Meaning of Maximum M

There are two distinct concepts of the maximum meaningful M.

### Theoretical maximum
The theoretical maximum M is the number of candidate models (e.g., 120 or 240 after filtering). The function allows M to grow this large.

### Useful maximum
Under the hard-assignment rule (each participant picks the best single architecture), the fully individualized optimum is achieved as soon as the library includes every participant's personal best model. The number of unique personal best models is `M_useful_max ≤ N_participants`. Beyond this M, adding more architectures cannot improve the score under any globally optimal search. The function computes and reports `M_useful_max` and `n_unique_individual_best_models` as output columns. If a greedy search improves the score beyond `M_useful_max`, this signals a search approximation issue and triggers a diagnostic warning.
...
Report the M flagged by each criterion. If all four agree, the answer is robust. If they disagree, the curve is gradual and no single M is unambiguously "the" answer — which is itself a finding worth reporting.

**"What does A(M) = 0.6 at M=2 mean?"**
...
**"If M=2 doesn't include the IC champion, is that a problem?"**
No — it is scientifically interesting. It means the population IC champion is a compromise between two clusters, each better described by a different architecture. The optimal M=2 library may describe structural heterogeneity more accurately than forcing the IC winner into the picture.
...
BIC penalizes parameter complexity within each model. The IC-equivalent scoring ensures the M=1 result matches the published IC analysis. The stopping criteria prevent choosing more M than the data support. Cross-validated H_form (computed separately) provides a held-out generalization check.
...
**"Does the Kneedle elbow depend on the choice of M_max, and if so, is that a problem?"**

Yes — the Kneedle distance is sensitive to M_max by construction, and this needs to be addressed explicitly in the paper.

*The math.* The Kneedle procedure normalizes M to [0, 1] using M_max as the right endpoint: M_norm = (M − 1) / (M_max − 1). The elbow is the M that maximizes the vertical distance of A(M) above the straight line connecting (M=1, A=0) to (M=M_max, A(M_max)). If M_max were smaller, the x-scaling would compress, each M would appear further to the right in normalized space, and the elbow would shift accordingly. The Kneedle elbow is therefore a statement about the shape of the curve *within the searched range*, not an absolute quantity.

*Why M_useful_max is the principled right endpoint.* The search was run to M_useful_max — the number of distinct utility architectures that are at least one participant's personal best fit (22 in this dataset). This is not an arbitrary cutoff chosen by the researcher. It is the exact M at which A(M) = 1 is first achievable: for the first time, every participant has their preferred architecture available in the library. Beyond M_useful_max, no reduction in population BIC is possible, so the curve is completely traced. The compression curve spans the full range from A(1) = 0 (forced to use one architecture for everyone) to A(M_useful_max) = 1 (everyone gets their preferred architecture), and the Kneedle elbow identifies the point of maximum benefit-to-cost ratio within that complete range. This is the correct scientific framing.

*How to write this in the paper.* Something like: "We searched from M = 1 to M_useful_max = 22, where M_useful_max is the number of architectures that are at least one participant's best-fitting model — the first M at which the fully individualized BIC optimum is achievable. The Kneedle elbow was computed over this complete range, so it identifies the M that captures the greatest fraction of the individualized advantage per additional architecture relative to the theoretical maximum. Because M_useful_max is data-determined rather than researcher-chosen, the elbow is anchored to a principled endpoint."

*Additional robustness.* Two of the other stopping criteria — marginal gain (ΔA < 1%) and maximum curvature (largest Δ²A) — do not depend on M_max at all; they are purely local measures of the curve's shape. If those criteria agree with the Kneedle result, the elbow is robust to the M_max dependence. Report all criteria and note agreement or disagreement.

*The one residual limitation.* Even with a principled M_max, a reviewer could note that M_useful_max depends on sample size: with more participants, more unique personal bests would exist, potentially raising M_useful_max. The appropriate response is that M_useful_max is descriptive of this sample and population (it characterizes the actual diversity of best-fitting models in these data), and that the compression curve describes the tradeoff between parsimony and fit for this population — which is exactly the quantity of scientific interest.

---

## Appendix: Empirical Findings from the Compression Curve Analysis (Experiment Data Notes)

*These notes record observations made during analysis of the actual data and are intended to inform paper writing. Numbers are specific to this dataset (N = 73 participants, M_useful_max = 22).*

---

### Finding 1: The M = 2 jump is the headline result (ΔA = 0.58)

Going from one shared utility function to two captures **58% of the total individualized advantage** in a single step. This is a large and important finding. A ΔA of 0.58 at M = 2 means that the population is genuinely bimodal in utility function type — the single IC-winning model is a poor descriptor of approximately half of all participants. The compression curve argues forcefully that a split library has large scientific value over a single consensus model, even before asking which models belong in that library.

---

### Finding 2: The IC winner is a statistical compromise, not anyone's best model

At M = 1, the IC-winning model (index 443) is assigned to all 73 participants and achieves a mean individual BIC of 131.7. When M = 2 allows participants to self-select into the library that fits them best, the 36 participants assigned to model 443 have a mean individual BIC of **142.0** — *worse* than the 37 participants assigned to model 455, who average **118.3**. The participants who "stay" with the IC winner are actually worse fit by it than the defectors are by their alternative.
...
One of the clearest structural patterns in the data is that **model 443 (the IC winner) appears in every optimal library from M = 1 to M = 22 without exception**. Its pruning cost remains high throughout — even at M = 8, removing it would cost a normalised BIC increase of approximately 0.08–0.46. It is not merely a legacy presence from M = 1; it is the genuine nucleus of every library, consistently assigned to 18–37% of participants at all levels.

By contrast, some models that enter later are striking for how well they fit their small assigned groups. **Model 452** achieves a mean individual BIC of **82–87** across all M values where it appears — exceptionally low, indicating that the participants who prefer it are captured almost perfectly by its functional form. **Model 373**, which first enters at M = 5, achieves a mean individual BIC of **79–80** for its 4 assigned participants. These are boutique models: they serve tiny minorities with extraordinary precision. The flip side is that their small coalitions make them hard to justify on parsimony grounds alone. They serve real people extremely well; they just don't serve many of them.

---

### Finding 5: The nested structure of optimal libraries, and the single exception

Across all M from 1 to 22, the optimal library at M is the optimal library at M − 1 plus exactly one new model. Library(4) is library(3) plus model 473. Library(5) is library(4) plus model 373. This nested, additive structure holds without exception from M = 3 through M = 22.

**The single exception is M = 2**, where model 455 appears — and then vanishes entirely from M = 3 through M = 8, before re-emerging at M = 9. This is the most structurally unusual feature of the entire compression curve.

---

### Finding 6: Model 455 as a big-tent coalition model — and the spoiler effect

Model 455's appearance at M = 2 and subsequent disappearance is not because a behaviourally similar cousin edges it out (though 452 and 455 do have a moderate AMPD of ~0.036, at the 32nd percentile). It is a more interesting phenomenon: **455 is functioning as a big-tent coalition model at M = 2**.

At M = 2, model 455 must cover everyone who is not best described by model 443. That is a heterogeneous group — participants who prefer reference-dependent utility, participants who prefer a pure-exponents form, and others who simply deviate from the 443 functional structure in different ways. Model 455 is the single best umbrella for all of these dissimilar people because it sits in a region of functional space that is tolerable (if not optimal) for each of them.

When M = 3 becomes available, the exhaustive search finds a better arrangement: the optimal library is {443, 452, 469}. Models 452 and 469 **split 455's constituency**. Model 452 is taken by participants it fits extremely well (mean BIC ~87); model 469 absorbs the remainder. This is a divide-and-conquer displacement, and the analogy to a spoiler effect in voting is apt: at M = 2, model 455 wins because it is the strongest single alternative to the incumbent (443). But when a third option enters, the constituency that was holding 455 together fragments — each sub-group defects to the specialist that better represents them. Model 455 loses not because it is bad, but because the electorate it was unifying was always internally divided, and M = 3 gives that division room to express itself.

Model 455 re-emerges at M = 9 because, by that point, all of the niches that were competing for its constituency are already filled by other specialists. The participants who were 455's natural constituency — those genuinely best described by its reference-dependent structure — are finally returned to it, freed from being absorbed by 452 or 469.

The implication for reporting: recommending model 455 to applied researchers at M = 2 should come with a caveat. It is acting as a coalition structure at that level of granularity. The M = 2 library captures the most parsimonious acknowledgement that the population is heterogeneous, but the specific pair {443, 455} does not reflect the stable, fine-grained structure that emerges at M ≥ 3. Researchers who need a practically useful split library are better guided toward M = 3 or M = 5, where the nested structure has stabilised and each model in the library is serving a more coherent constituency.

---

### Finding 7: Model 469 as a poor-fit placeholder

Model 469 has a mean individual BIC of 180–209 throughout its presence in the library (M = 3 onwards), far above the population mean and far above its co-members. The participants assigned to it are not well described by it — they are assigned to 469 because it is the least-bad option available in the library, not because it is a good fit. These participants are genuine outliers in utility function space; no model in the candidate pool captures them well.

The natural question is why the optimal M = 3 library is {443, 452, 469} rather than {443, 452, 473}, which would seem to replace the poorly-fitting 469 with a more competitive alternative. The answer is that the exhaustive search over all C(N_candidates, 3) combinations found that 469 specifically minimises total population BIC at M = 3 — meaning there is a sub-group of participants for whom 469 is decisively better than any other available model except 443 or 452. The 23 participants assigned to 469 at M = 3 are not served badly by 469 in an absolute sense unique to 469; they are poorly served by the entire top of the candidate pool, and 469 happens to be least-bad for them. These are participants whose utility structures may be genuinely idiosyncratic and worth examining individually.
...
The four stopping criteria return four different M values:

- **Max curvature and meta-BIC**: M = 3 (steepest deceleration of improvement; smallest penalised library size)
- **Kneedle elbow**: M = 5 (greatest departure from linearity in the compression curve)
- **Marginal gain (ΔA < 1%)**: M = 8 (last M delivering at least 1% additional gain)
- **Cumulative gain (A ≥ 80%)**: M = 4

This disagreement is not a bug or a limitation of the analysis — it is a substantive finding. **There is no uniquely correct answer to how many utility function types the population needs**, because that answer depends on how strongly one values parsimony relative to fit. A theorist who believes that one or two functional forms should suffice on parsimony grounds will stop at M = 3. An empiricist who wants the library to capture minority preference structures will continue to M = 5 or M = 8. Neither position is wrong; they reflect different values about what a model is for.

This is why the paper should present the full compression curve and report all stopping criteria rather than nominating a single winner. The appropriate framing is to give researchers a menu: here is what you gain at M = 1, 2, 3, 4, 5 — choose the library that reflects your own fit-parsimony priorities. The lack of criterion consensus is itself evidence that the curve declines gradually enough that no single M stands out as obviously optimal, which is a scientifically honest and useful thing to communicate.

---

### Note for the paper: the relative vs. absolute fit distinction

**This must be acknowledged explicitly.** A(M) is a *relative* quantity: it measures what fraction of the individualization gap is recovered by a M-architecture library. A(M=2) = 0.58 means that two architectures recover 58% of the advantage you would gain by giving every participant their own best-fitting model. But this says nothing about how large that advantage is in absolute terms.

If the fully individualized BIC advantage over the population winner (score_K1 − score_fully_individualized) is small in absolute terms, then 58% of a small gap is still a small absolute improvement. A(M=2) = 0.58 can be simultaneously true and practically unimportant if the underlying gap is modest.
...
**Recommended framing for the paper:** Report A(M) as the main compression curve quantity, but immediately annotate it with the absolute BIC improvement (score_K1 − score_M) in units readers can interpret — e.g., as average per-participant BIC improvement, or as improvement in held-out NLL per game. This lets readers judge whether the relative gain translates into a practically meaningful improvement in model fit. The relative gain answers "how much of the available individualization is captured?" The absolute gain answers "is individualization worth pursuing at all?"

---

### Finding 9: H(M) calibration — model 443 explains 95.4% of the explainable prediction improvement

**Terminology note — H(M) vs H_form:** These are two distinct measures and must not be confused.
- **H(M)** (this document, the paper quantity): fixed denominator. `H(M) = (BIC_chance − score_M) / (BIC_chance − score_fully_individualized)`. Both BIC_chance and score_fully_individualized are constants; H(M) is strictly bounded [0, 1].
- **H_form** (cross-validated analysis, architecture_codebook_summary.csv): moving denominator. `H_form = (NLL(1) − NLL(M)) / (NLL_chance − NLL(M))`. The denominator changes with M, so H_form does not share the same [0,1] interpretation and the two measures diverge, especially at small M.

**The key empirical result** (N = 73 participants, ~6,880 total game observations):

Define BIC_chance = 2 × n_total_games × log(2) — the BIC of a model that predicts 50/50 for every binary choice (0 free parameters). Define H(M) as:

```
H(M) = (BIC_chance − score_M) / (BIC_chance − score_fully_individualized)
```

H(M) is bounded [0, 1]: H = 0 means performance at chance level; H = 1 means performance at the fully individualized ceiling (best possible fit given the 480-model universe — NOT zero prediction error, which is unachievable). The upper bound is total individualization, not perfect prediction.

**Empirical values (this dataset):**

| M | H(M) | Interpretation |
|---|------|----------------|
...
The total individualization budget (BIC_chance − score_fully_individualized ≈ 4,555 BIC units across all participants) is only 4.6% unexplained by model 443. The "58% jump" from M=1 to M=2 — often cited as evidence of meaningful functional heterogeneity — represents 58% of that 4.6%, which is a 2.7 percentage-point absolute improvement.

**The headline finding for the paper:** Model 443 already leaves only 4.6% of explainable prediction improvement on the table. The case for using multiple utility function architectures rests on whether that 4.6% matters for the researcher's purpose. For population-level descriptive inference, M=1 is almost certainly adequate at H = 95.4%. For individual-level prediction or precision tailoring, M=2 (H = 98.1%) or M=3 (H = 99.0%) may be warranted. This gives researchers a principled anchor: choose M based on the H threshold your research question requires.

**Equivalent framing:** 1 − H(M=1) = 0.046 measures the residual unexplained gap on a scale from 0 (perfect = fully individualized) to 1 (chance). Model 443 sits 4.6% away from the fully individualized ceiling.

**H(M) column in summary table:** H_M and absolute_gap_closed_versus_chance = BIC_chance − score_M are now saved as columns in population_architecture_summary_table.csv, placed immediately before the utility_idx column so they appear in the repeated-M block alongside A_M and delta_A_M. The architecture.py code now computes these at runtime (see the H_M block after the meta_bic stopping criterion). H_form_M in architecture_codebook_summary.csv is a different, cross-validated quantity — see the terminology note in Finding 9.
