...
Read this before writing or suggesting any code changes. Do not overwrite this file. 
...
`utility_settings` is a dict of 17 boolean toggles that selects among ~505 possible utility
...
### kwargs-override-general_settings pattern

Many analysis functions accept explicit kwargs that mirror keys nested inside `general_settings`.
The convention is: the kwarg defaults to `None`. When `None`, the function reads the value from
`general_settings`. When the caller passes an explicit value, that value wins.

```python
def compute_something(..., n_workers=None, score_basis=None):
    s = general_settings.get('individual_architecture_settings', {})
    if n_workers   is None: n_workers   = s.get('n_workers',   None)
    if score_basis is None: score_basis = s.get('score_basis', 'ic_equivalent_participant_score')
```

This is the same pattern used for `create_new_file` throughout the codebase (see
generate-cache-retrieve pattern below). When writing a new function that wraps a
`general_settings` sub-dict, follow this exactly — no custom sentinel objects.
